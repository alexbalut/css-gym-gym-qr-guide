...
Seed gym: `css-gym`, `primaryColor: #00897B`, `secondaryColor: #000000`.

## Multi-tenant gym

Admin session carries `gymId`. APIs scope mutations to that gym. Member home loads slug `css-gym` (fallback: first gym).
...
| Workout + history | localStorage keys `css-gym-workout:v1:<slug>` and `css-gym-workout-history:v1:<slug>` |
