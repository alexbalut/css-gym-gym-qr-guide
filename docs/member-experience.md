...
Members never log in. `/` is the in-product gym home for **Coop Sportive Santé**.
...
3. Friendly URLs: `/m/css-gym/lat-pulldown`.
...
Keyed by gym slug (`css-gym`). No member account.

| Key | Contents |
|-----|----------|
| `css-gym-workout:v1:<slug>` | In-progress session JSON |
| `css-gym-workout-history:v1:<slug>` | Array of saved workouts (newest first) |
